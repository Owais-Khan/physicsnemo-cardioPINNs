...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    if (cgsFactor is not None):
        ...
        #Normalize the inlet/outlet/interior meshes
        print (interior_mesh_vtk.GetBounds())
...
if __name__ == "__main__":
    ...
    for i in range(0,len(velocity_files)):
        ...
        """#Directory
        Directory="outputs/"+os.path.splitext(os.path.basename(__file__))[0]

        #Re-Scale the Inference File
        print ("\n"+"-"*30)
        print ("Re-Transforming the Inference Data to Original Coordinates")
        infile=open(os.path.join(Directory,"TransformationParameters.dat"),'r')
        Centroid=[float(value) for value in infile.readline().split()[1:]]
        ScalingFactor=float(infile.readline().split()[-1])
      
        #Read the Inference File
        VelocityFile=ReadVTUFile(os.path.join(Directory,"inferencers/%s"%os.path.basename(VelocityFilePath)))
        
        #Translate the VelocityFile back to original coordinates
        VelocityFileReNormalized=reverse_normalize_mesh_vtk(VelocityFile,Centroid,ScalingFactor)
        
        #Write the VTU File
        WriteVTUFile(os.path.join(Directory,"inferencers/%s"%os.path.basename(VelocityFilePath)),VelocityFileReNormalized)"""
