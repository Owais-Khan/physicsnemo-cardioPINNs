...
def run(cfg: PhysicsNeMoConfig) -> None:
    ...
    """if (NormalizeInput is True):
        print ("\n"+"-"*30)
        print ("Normalizing the Velocity using Input Data")
        U_mean=0; V_mean=0; W_mean=0; VelMag=0
        for velocity_data_ in velocity_data:
            velocity_data_np_=vtk_to_numpy(velocity_data_.GetPointData().GetArray(VelocityArrayName))
            U_mean+=np.average(velocity_data_np_[:,0])
            V_mean+=np.average(velocity_data_np_[:,1])
            W_mean+=np.average(velocity_data_np_[:,2])
            velmag_=np.linalg.norm(velocity_data_np_,axis=1)
            VelMag+=np.average(velmag_[velmag_!=0])
        U_mean=U_mean/len(velocity_data)
        V_mean=V_mean/len(velocity_data)
        W_mean=W_mean/len(velocity_data)
        if VelocityScale is None: VelocityScale=VelMag/len(velocity_data)
        print ("--- U_mean: %.05f"%U_mean)
        print ("--- V_mean: %.05f"%V_mean)
        print ("--- W_mean: %.05f"%W_mean)
        print ("--- VelocityScale: %.05f"%VelocityScale)"""
    ...
    if (NormalizeInput is True):
        print ("\n"+"-"*30)
        print ("Normalizing the Input Mesh Using Bounding Box")
        BBSize=np.sqrt(xRange**2+yRange**2+zRange**2)
        if MeshScale is None: MeshScale=1/BBSize
        ...
        print ("--- Centroid is (%.05f, %.05f, %.05f)"%(MeshCentroid[0],MeshCentroid[1],MeshCentroid[2]))
        print ("--- Bounding Box Size is: %.05f"%BBSize)
        print ("--- MeshScaling is: %.05f"%MeshScale)


        #Normalize the Velocity Mesh
        velocity_data_normalized=[normalize_mesh_vtk(velocity_data_,MeshCentroid, MeshScale) for velocity_data_ in velocity_data]
...
#---------------------------- Navier-Stokes ----------------------------------#
    ...
    ns = NavierStokes(nu=nu * MeshScale, rho=1.0, dim=3, time=True)
    #Flow Net
    ...
    #Time Window
    print ("--- Creating Time Window node...")
...
#---------------------------- Initial Conditions -----------------------------#
    ...
    #Get the velocity at initial time steps
    velData_invar_0, velData_outvar_0=CardioPINNsGetVelocityData(velocity_data_normalized[0],VelocityArrayName,DistanceThresholdPercentile)
   ...
   print ("--- Creating Zero-Pressure Boundary Condition at Inlet...") 
    ...
    #Data Loss (WORK IN PROGRESS)
    """for i, specific_time in enumerate(np.linspace(0,time_window_size,nr_time_windows)):
        velData_invar, velData_outvar=CardioPINNsGetVelocityData(velocity_data_normalized[i],VelocityArrayName,DistanceThresholdPercentile)
