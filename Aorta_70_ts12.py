...
def run(cfg: PhysicsNeMoConfig) -> None:
	...
	point_path = to_absolute_path("/home/khanmu11/Data/physicsnemo-cardioPINNs/Data1_Stanford4DFlowMRI/")
	MeshPath=to_absolute_path("/home/khanmu11/Data/physicsnemo-cardioPINNs/Data1_Stanford4DFlowMRI/mesh-complete.mesh.vtu")
	...
	Velocity90th=torch.quantile(torch.sqrt(torch.square(torch.tensor(velData_outvar["u"]))	+ torch.square(torch.tensor(velData_outvar["v"])) + torch.square(torch.tensor(velData_outvar["w"]))),0.85)
	
	#Adjust the velocity since it is sampled along the centerline and thus higher (can't assume Vmean=Vmax/2)
	VelocityMeanAdjusted=Velocity90th/2. #(VelocityMean/(2-0.01*RadiusThreshold))
	InflowRate=VelocityMeanAdjusted*inlet_area #Estimate inlet flow rate

	print ("\n"+"-"*30)
	print ("--- Number of Sampled Points:			%d"%NumberOfVelPoints)
	print ("--- Mean Velocity Magnitude of Sampled Points:	%.05f"%VelocityMean)
	print ("--- Median Velocity Magnitude of Sampled Points:%.05f"%VelocityMedian)
	print ("--- Estimated InflowRate is:			%.05f"%InflowRate)

	
	# make domain for geometry
	domain = Domain()
#---------------------------- Navier-Stokes ----------------------------------#
	print ("\n"+"-"*30)
...
#---------------------------- Constraints -----------------------------#
	...
	inlet = PointwiseBoundaryConstraint(
		...
		lambda_weighting={"u":0.01*(100-RadiusThreshold), "v":0.01*(100-RadiusThreshold), "w":0.01*(100-RadiusThreshold)}
	)
	...
	integral_continuity = IntegralBoundaryConstraint( 
		nodes=nodes,
		geometry=inlet_mesh,						 #Inlet cap					 
		outvar={"normal_dot_vel": -1*InflowRate},	 #Integration of velocity is flow rate. 
		batch_size=1,
		integral_batch_size=cfg.batch_size.integral_continuity,					 
		lambda_weighting={"normal_dot_vel": (VelocityMeanAdjusted/InflowRate)*0.01*(100-RadiusThreshold)}, #weight more for less data. 
		)					   
	...
	for i in range(len(outlet_mesh)):
		...
		integral_continuity = IntegralBoundaryConstraint( 
			...
			batch_size=1,
			integral_batch_size=cfg.batch_size.integral_continuity, 
			lambda_weighting={"normal_dot_vel": VelocityMeanAdjusted/flow_rate_*0.01*(100-RadiusThreshold)},
		)																																						   
		domain.add_constraint(integral_continuity, "Integral_%s"%os.path.splitext(os.path.basename(outlet_path[i]))[0])


	print ("\n--- Creating Interior Boundary Conditions (Continuity, Momentum)...")
...
#----------------------------------- Add Monitors to Output ------------------------------
	...
	vtk_obj = VTKFromFile(os.path.join(VelocityFilePathVTU),export_map={VelocityArrayName+"_PINNs": ["u", "v", "w"], "pressure_PINNs": ["p"]},)
	...
	velData_invar_PINNs, velData_outvar_PINNs=CardioPINNsGetVelocityData(VelocityDataPINNs,VelocityArrayName+"_PINNs",DistanceAwayFromWall) #Extract the Same Points as used in DataLoss
...
if __name__ == "__main__":
	RadiusThreshold=70 #What percentange of lumen to sample along centerline. 90% = sample 90% of lumen 
